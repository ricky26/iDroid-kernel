...
#define h2fmi_set_emf(a,b) {}
#define h2fmi_get_emf() 0
#define nand_device_set_ftl_region(a, b, c, d, e) {}

uint64_t divide64(uint64_t n, uint32_t base)
{
	uint64_t tmp = n;
	do_div(tmp, base);
	return tmp;
...
static uint32_t BTOC_Init(void)
{
	uint32_t i;
	...
	if (bufzone_finished_allocs(&sInfo.btocCacheBufzone))
		return ENOMEM;
	...
	if (bufzone_finished_rebases(&sInfo.btocCacheBufzone))
		return ENOMEM;

	return 0;
}

static void updateBtocCache(void)
{
	uint32_t i;
	for (i = 0; i < sInfo.numBtocCaches; ++i) {
...
static uint32_t YAFTL_Init(void)
{
	...
	printk(KERN_ERR "numBlocks: %d\r\n", sGeometry.numBlocks);
	printk(KERN_ERR "bytesPerPage: %d\r\n", sGeometry.bytesPerPage);
	printk(KERN_ERR "total_banks_ftl: %d\r\n", sGeometry.total_banks_ftl);
	printk(KERN_ERR "metaBytes: %d\r\n", metaBytes);
	printk(KERN_ERR "unkn20_1: %d\r\n", unkn20_1);
	printk(KERN_ERR "total_usable_pages: %d\r\n", sGeometry.total_usable_pages);
	...
	if (bufzone_finished_rebases(&sInfo.zone))
		return -1;
...
static void initBlockStats(void)
{
	uint32_t i;
...
static int writeCxtInfo(uint32_t _page, void* _buf)
{
	...
	// Write statistics
	sVSVFLStats = vfl->get_stats(vfl, &vSize);
...
static int flushQuick(void)
{
	...
	if (tempBuf == NULL) {
		printk(KERN_ERR "yaftl: flushQuick out of memory\r\n");
		return ENOMEM;
	}
	...
	vfl_erase_single_block(vfl, block, 1);
	++sInfo.blockArray[block].eraseCount;
...
void YAFTL_Flush(void)
{
	...
	for (block = 0; block < sGeometry.numBlocks && found < 3; block++) {
		// Must be a kfree block, with much better erase count.
		if (!sInfo.blockArray[block].status == BLOCKSTATUS_FREE
	...
	for (i = 0; i < 3; ++i) {
		block = oldBlks[i];
		vfl_erase_single_block(vfl, oldBlks[i], 1);
...
FLUSH_QUICK:
	while (flushQuick() != 0)
		printk(KERN_ERR "yaftl: flushQuick failed\r\n");
...
static uint32_t YAFTL_readCxtInfo(uint32_t _page, uint8_t* _ptr, uint8_t _extended, uint32_t *pSupported)
{
	...
	if (YAFTL_readPage(_page, _ptr, spare, 0, 1, 0))
		return ERROR_ARG;
	...
	if (memcmp(pCxt->version, "CX01", 4)) {
		...
		printk(KERN_ERR "yaftl: Wrong context version: %s\r\n", (char*)pCxt);
		return ERROR_ARG;
	}
	...
	if (_extended) {
		...
		for (i = 0; i < sInfo.tocPagesPerBlock; i++) {
			if (YAFTL_readPage(pageToRead + i,
				&buf[i * sGeometry.bytesPerPage], spare, 0, 1, 0)
	...
	// Read statistics.
	if (YAFTL_readPage(_page + sInfo.tocPagesPerBlock*2 + 1, _ptr, spare, 0,
		1, 0) || !(spare->type & PAGETYPE_FTL_CLEAN))
	{
		sInfo.field_78 = 0;
		return ERROR_ARG;
	}
	...
	if (_extended) {
		...
		for (i = 0; i < sInfo.nPagesTocPageIndices; i++) {
			if (YAFTL_readPage(pageToRead + i, _ptr, spare, 0, 1, 0)
				|| !(spare->type & PAGETYPE_FTL_CLEAN))
			{
				sInfo.field_78 = 0;
				return ERROR_ARG;
...
static BlockListNode *addBlockToList(BlockListNode *listHead, uint32_t blockNumber, uint32_t usn)
{
	...
	if (listHead) {
		...
		if (usn < curr->usn) {
			...
			if (usn == curr->usn)
				printk(KERN_ERR "yaftl: found two blocks (%d, %d) with the same usn.\r\n", blockNumber, curr->blockNumber);
...
static uint32_t restoreIndexBlock(uint16_t blockNumber, uint8_t first)
{
	...
	if (!result && !(spare->type & PAGETYPE_INDEX)) {
		printk(KERN_ERR "yaftl: restoreIndexBlock called with a non-index block %d.\r\n", blockNumber);
		return 0;
	}
...
static uint32_t YAFTL_Restore(uint8_t ftlCtrlBlockPresent)
{
	...
	if (!tempBuffer) {
		printk(KERN_ERR "yaftl: out of memory.\r\n");
		return EIO;
	}
	...
	for (i = 0; i < sGeometry.numBlocks; i++) {
		...
		// If no readable page was found, kfree this block.
		if (result > 1) {
	...
	// If we haven't found any index block, use a kfree one.
	...
	if (userListHead) {
		...
		for (i = 0; i < sInfo.totalPages; i += entriesInBootBuffer) {
			...
			pagesInBootBuffer = entriesInBootBuffer / sInfo.tocEntriesPerPage;
			...
			for (j = 0; j < pagesInBootBuffer && (i + j * sInfo.tocEntriesPerPage) < sInfo.totalPages; j++) {
				...
				if (memcmp(&bootBuffer[bootBufferOffset], tempBuffer, sGeometry.bytesPerPage) != 0) {
					if (indexPage != 0xFFFFFFFF && YAFTL_readPage(indexPage, tempBuffer, 0, 0, 1, 0) != 0) {
						printk(KERN_ERR "yaftl: index page is unreadable at 0x%08x\r\n", indexPage);
						memset(tempBuffer, 0xFF, sGeometry.bytesPerPage);
					}
					...
					if (memcmp(&bootBuffer[bootBufferOffset], tempBuffer, sGeometry.bytesPerPage) != 0) {
						...
						if (cacheNum == 0xFFFF) {
							kfree(bootBuffer);
							kfree(blockLpns);
							printk(KERN_ERR "yaftl: out of TOC caches, but restore might have already succeeded.\r\n");
							goto restore_final;
						}

						tocCache = &sInfo.tocCaches[cacheNum];
		...
		// No user pages. Find a kfree block, and choose it to be the latest user page.
...
restore_final:
	...
	printk(KERN_ERR "yaftl: restore done.\r\n");
...
static uint32_t YAFTL_Open(uint32_t signature_bit)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (result == 0) {
			...
			/*
			vfl_erase_single_block(vfl, ftlCtrlBlockBuffer[i], 1);
		...
		while (1) {
			...
			if (i >= sGeometry.pagesPerSublk - sInfo.ctrlBlockPageOffset
				|| YAFTL_readPage(sGeometry.pagesPerSublk * ftlCtrlBlock + i,
					sInfo.pageBuffer, spare, 0, 1, 0) != 0)
			{
				printk(KERN_ERR "yaftl: no valid context slot was found, a restore is definitely needed.\r\n");
			...
			if (YAFTL_readPage(sGeometry.pagesPerSublk * ftlCtrlBlock + i
					+ sInfo.ctrlBlockPageOffset, sInfo.pageBuffer, spare, 0,
					1, 0) == 1)
			{
	...
	printk(KERN_ERR "yaftl: ftl context is at logical page %d\r\n", sInfo.ftlCxtPage);

	if (versionSupported != 1) {
		printk(KERN_ERR "yaftl: unsupported low-level format version.\r\n");
		return EINVAL;
	}

	if (sStats.refreshes == 0xFFFFFFFF) {
		printk(KERN_ERR "yaftl: clearing refresh stats\r\n");
		sStats.refreshes = 0;
	}
	...
	if (!sInfo.tocCaches) {
		printk(KERN_ERR "yaftl: error allocating memory\r\n");
		return -1;
	}
	...
	if (bufzone_finished_allocs(&sInfo.segment_info_temp))
		return -1;
	...
	if (bufzone_finished_rebases(&sInfo.segment_info_temp))
		return -1;
	...
	printk(KERN_ERR "yaftl: successfully opened!\r\n");
...
static int verifyUserSpares(uint32_t _start, SpareData* _spares, size_t _count)
{
	...
	for (i = 0; i < _count; i++) {
		...
		{
			printk(KERN_ERR "YAFTL_Read: mismatch between lpn and metadata!\r\n");
			return 0;
		}
...
#endif
	int ret = 0;
	uint32_t tocPageNum, tocEntry, tocCacheNum, kfreeTOCCacheNum;
	...
	while (pagesRead != nPages) {
			...
			&& (sInfo.tocArray[tocPageNum].indexPage == 0xFFFFFFFF))
		{
			...
			if (tocCacheNum != 0xFFFF) {
				...
				if ((kfreeTOCCacheNum = YAFTL_findFreeTOCCache()) != 0xFFFF)
				{
					// There's a kfree cache.
					ret = YAFTL_readPage(
							sInfo.tocArray[tocPageNum].indexPage,
							(uint8_t*)sInfo.tocCaches[kfreeTOCCacheNum].buffer,
							0, 0, 1, 1);
					...
					L2V_UpdateFromTOC(tocPageNum,
						sInfo.tocCaches[kfreeTOCCacheNum].buffer);
					sInfo.numFreeCaches--;
					sInfo.tocCaches[kfreeTOCCacheNum].state = CACHESTATE_CLEAN;
					sInfo.tocCaches[kfreeTOCCacheNum].useCount = 1;
					sInfo.tocCaches[kfreeTOCCacheNum].page = tocPageNum;
					sInfo.tocArray[tocPageNum].cacheNum = kfreeTOCCacheNum;

					tocEntry = sInfo.tocCaches[kfreeTOCCacheNum].buffer[(lpn + pagesRead) % sInfo.tocEntriesPerPage];
				} else {
					// No kfree cache. Is this the last TOC page we read?
...
static uint32_t* refreshTOCCaches(uint32_t _page, uint32_t* _pCacheEntry)
{
	...
	uint32_t kfreeCacheSlot;

	*_pCacheEntry = _page % sInfo.tocEntriesPerPage;

	if (cacheNum != 0xFFFF) {
		kfreeCacheSlot = cacheNum;
	} else {
		// Not cached. Retreive a kfree cache.
		kfreeCacheSlot = YAFTL_findFreeTOCCache();

		if (kfreeCacheSlot == 0xFFFF) {
			// No kfree caches. Clear one and use it.
	...
	sInfo.tocCaches[kfreeCacheSlot].page = _page / sInfo.tocEntriesPerPage;
	sInfo.tocArray[_page / sInfo.tocEntriesPerPage].cacheNum = kfreeCacheSlot;

	sInfo.tocCaches[kfreeCacheSlot].state = CACHESTATE_DIRTY;
	++sInfo.tocCaches[kfreeCacheSlot].useCount;

	if (indexPage != 0xFFFFFFFF) {
		if (cacheNum == 0xFFFF) {
			// Read the index page into the newly obtained cache.
			error_t result = YAFTL_readPage(indexPage,
					(uint8_t*)sInfo.tocCaches[kfreeCacheSlot].buffer, pSpare,
					0, 1, 1);
	...
	return sInfo.tocCaches[kfreeCacheSlot].buffer;
}
...
static void invalidatePages(uint32_t _start, uint32_t _count)
{
	...
	// Collect garbage for each block which should be kfreed.
...
static int YAFTL_Write(uint32_t _pageStart, uint32_t _numPages, uint8_t* _pBuf)
{
	...
	while (!finished) {
		...
		while (toWrite != 0) {
			...
			result = 0;
			for (i = 0; i < maxWritePages && !result; ++i) {
				...
				result = vfl_write_single_page(vfl, page,
						...
						(uint8_t*)&sInfo.spareBuffer19[i], 1);
			}
			...
			if (result) {
				...
				if (sInfo.blockArray[sInfo.latestUserBlk.blockNum].status
						!= BLOCKSTATUS_FREE) {
					gcFreeBlock(sInfo.latestUserBlk.blockNum, 1);
				}
			...
			// This iteration was successful, so set finished to 1.
			finished = 1;
...
error_t ftl_yaftl_read_single_page(ftl_device_t *_ftl, uint32_t _page, uint8_t *_buffer)
{
	...
	return ret;
#else
	return YAFTL_Read(_page, 1, _buffer);
#endif
}
...
error_t ftl_yaftl_write_single_page(ftl_device_t *_ftl, uint32_t _page, uint8_t *_buffer)
{
	...
	return ret;
#else
	return YAFTL_Write(_page, 1, _buffer);
#endif
}
...
{
	ftl_yaftl_device_t *ret = kmalloc(sizeof(ftl_yaftl_device_t), GFP_KERNEL);
